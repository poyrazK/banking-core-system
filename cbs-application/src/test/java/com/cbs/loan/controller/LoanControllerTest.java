...
class LoanControllerTest {
    ...
    void repayLoan_returnsBusinessErrorWhenNotDisbursed() throws Exception {
        when(loanService.repayLoan(any(), any()))
                .thenThrow(new ApiException("LOAN_NOT_DISBURSED", "Repayments are allowed only for disbursed loans"));
        ...
        mockMvc.perform(patch("/api/v1/loans/{loanId}/repay", 99)
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("Repayments are allowed only for disbursed loans"));
    }
